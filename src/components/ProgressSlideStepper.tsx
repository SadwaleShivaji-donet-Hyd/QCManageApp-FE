        ...
        return (
          <div key={step} className="flex items-center flex-1 min-w-[8px]">
            ...
            <div className="relative flex items-center shrink-0">
              {isComplete ? (
                <div className="bg-[#00ACC1] rounded-full w-4 h-4 flex items-center justify-center overflow-hidden">
                  <Check size={10} className="text-white" strokeWidth={3} />
                </div>
              ) : (
                <div className="bg-[#E2E8F0] rounded-full w-4 h-4 border border-[#94A3B8]" />
              )}

              {isActive && (
                <div
                  className="absolute -top-2.5 -left-12.5 whitespace-nowrap bg-white flex items-center justify-center px-2 py-1.5 rounded-lg shrink-0 text-xs"
                  style={{ border: "2px solid #4338E0" }}
                >
                  <span className="text-[#1E3A5F] text-xs whitespace-nowrap">
                    {step}
                  </span>
                </div>
              )}
